...
app.post("/accept-order/:id", async (req, res) => {
  try {
    ...
    // Modify URLs if on-premises hosting
    if (
      (order.versioningTool === "Gitlab" || order.versioningTool === "Azure DevOps") &&
      order.hostingType === "On-Premises"
    ) {
    ...
    // Check the status of the workflow asynchronously in the background
    checkWorkflowStatus(order.id, workflowRunsUrl);

    // Respond to the client immediately without waiting for the workflow completion
    return res.status(200).json({
...
// Function to check the workflow status asynchronously
const checkWorkflowStatus = async (orderId, workflowRunsUrl) => {
  try {
    ...
    // Fetch the workflow runs to check for the latest completed one
    const { data } = await axios.get(workflowRunsUrl, {
      headers: {
        Authorization: `Bearer ${GITHUBTOKEN}`,
        Accept: "application/vnd.github.v3+json",
    ...
    // Now retrieve the repos created by the workflow to find the temp-repo
    const reposUrl = "https://api.github.com/user/repos";
    const { data: repos } = await axios.get(reposUrl, {
      headers: {
        Authorization: `Bearer ${GITHUBTOKEN}`,
    ...
    // You can send a notification, an email, or call a webhook with the repoUrl if needed.

  } catch (error) {
    console.error("Error while checking workflow status:", error.message);
  }
};
